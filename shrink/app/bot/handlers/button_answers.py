...
async def support_button_answer_handler(
    ...
    await state.clear()
    await audio_service.delete_extra_audio(user_id=message.from_user.id)
...
@inject
async def mailing_button_answer_handler(
    message: Message,
    ...
    if not registered:
        ...
        if subscription != "free":
            await message.answer(get_choose_type_of_mailing(), reply_markup=inline.choose_mailing_type_kb_markup)
        else:
...
@inject
async def main_menu_answer_handler(
    message: Message, 
    state: FSMContext,
    audio_service: Annotated[AudioService, Depends()],
) -> None:
